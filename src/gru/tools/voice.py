...
async def send_voice_message(
    chat_id: str, text: str, voice_id: str | None = None, speed: float | None = None, provider: str | None = None
) -> dict[str, Any]:
...
async def transcribe_voice_message(
    audio_data: str | bytes, format: str = "ogg", provider: str | None = None, language: str | None = None
) -> dict[str, Any]:
...
async def update_voice_settings(
    ...
    try:
        ...
        return {"status": "updated", "settings": await get_voice_settings()}
...
async def _generate_openai_speech(text: str, voice: str, speed: float) -> bytes | None:
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {_voice_config.openai_key}", "Content-Type": "application/json"}
        data = {"model": "tts-1", "input": text, "voice": openai_voice, "speed": speed}
...
async def _transcribe_audio(audio_bytes: bytes, format: str, provider: str, language: str) -> str | None:
    ...
    try:
        ...
        if provider == "claude":
            logger.warning(
                "Claude STT provider requested but not supported for audio transcription. "
                "Claude's API only supports PDF documents, not audio files. "
                "Falling back to supported provider."
            )
...
async def _send_audio_to_telegram(chat_id: int, audio_data: bytes) -> None:
    ...
    try:
        ...
        await telegram_bot._app.bot.send_voice(chat_id=chat_id, voice=audio_file)
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    return {"duration": duration, "format": format, "size": len(audio_bytes), "method": "ffprobe"}
            ...
            try:
                ...
                return {"duration": duration, "format": format, "size": len(audio_bytes), "method": "pydub"}
            except ImportError:
